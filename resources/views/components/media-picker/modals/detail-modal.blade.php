<x-media-picker.base-modal title="{{ __('media.detail.title') }}" maxWidth="4xl" :showFooter="false">
    <div class="grid gap-6 md:grid-cols-2">
        <!-- Left: Preview -->
        <div class="space-y-4">
            <div class="flex aspect-square items-center justify-center overflow-hidden rounded-lg border border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-900"
                ...
                @else
                    <div class="flex flex-col items-center justify-center p-8">
                        ...
                        <span
                            class="mt-4 text-sm text-gray-500 dark:text-gray-400">{{ $this->getDetailMedia()->mime_type }}</span>
                    </div>
                @endif
            </div>
            ...
            <div class="flex justify-center gap-2">
                <button wire:click="deleteDetailMedia" wire:confirm="{{ __('media.detail.delete_confirm') }}"
                ...
                <a href="{{ $this->getDetailMedia()->getUrl() }}" download="{{ $this->getDetailMedia()->file_name }}"
                    title="{{ __('media.detail.download') }}"
                    class="rounded-lg bg-gray-100 p-2 text-gray-600 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-400 dark:hover:bg-gray-600">
                ...
                <button onclick="navigator.clipboard.writeText('{{ $this->getDetailMedia()->getUrl() }}')"
                    title="{{ __('media.detail.copy_link') }}"
                    class="rounded-lg bg-gray-100 p-2 text-gray-600 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-400 dark:hover:bg-gray-600">
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <a href="{{ $this->getDetailMedia()->getUrl() }}" target="_blank"
                    title="{{ __('media.detail.open_in_new_tab') }}"
                    class="rounded-lg bg-gray-100 p-2 text-gray-600 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-400 dark:hover:bg-gray-600">
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label
                        class="text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">{{ __('media.detail.size') }}</label>
                ...
                @if ($this->imageInfo)
                    <div>
                        <label
                            class="text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">{{ __('media.detail.dimensions') }}</label>
                        <p class="mt-1 text-sm font-medium text-gray-900 dark:text-white">
                ...
                <div>
                    <label
                        class="text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">{{ __('media.detail.date') }}</label>
                    <p class="mt-1 text-sm font-medium text-gray-900 dark:text-white">
                        {{ $this->getDetailMedia()->created_at->format('n/j/Y') }}</p>
                </div>
                <div>
                    <label
                        class="text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">{{ __('media.detail.extension') }}</label>
                    <p class="mt-1 text-sm font-medium text-gray-900 dark:text-white">
                        {{ pathinfo($this->getDetailMedia()->file_name, PATHINFO_EXTENSION) }}</p>
                </div>
                <div class="col-span-2">
                    <label
                        class="text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">{{ __('media.detail.asset_id') }}</label>
                    <p class="mt-1 text-sm font-medium text-gray-900 dark:text-white">{{ $this->getDetailMedia()->id }}
                    </p>
                </div>
            </div>
            ...
            <div class="space-y-4">
                <!-- File name -->
                <div>
                    <label for="detail-filename"
                        class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('media.detail.file_name') }}</label>
                ...
                <div>
                    <label for="detail-alt"
                        class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('media.detail.alt_text') }}</label>
                    <textarea id="detail-alt" wire:model="detailAltText" rows="2"
                        class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-600 px-4 py-2.5 shadow-sm focus:border-primary-500 focus:ring-primary-500 dark:bg-gray-700 dark:text-white sm:text-sm"
                        placeholder="{{ __('media.detail.alt_text_placeholder') }}"></textarea>
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ __('media.detail.alt_text_hint') }}</p>
                </div>

                <!-- Caption -->
                <div>
                    <label for="detail-caption"
                        class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('media.detail.caption') }}</label>
                ...
                <div>
                    <label for="detail-location"
                        class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('media.location') }}</label>
                ...
                <div>
                    <label for="replacement-file"
                        class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('media.detail.replace_media') }}</label>
                    ...
                    @if ($this->replacementFile)
                        <p class="mt-1 text-xs text-green-600 dark:text-green-400">
                            {{ __('media.detail.new_file_selected') }}
    ...
    <div
        ...
        @if ($this->replacementFile)
            <button wire:click="replaceMedia" type="button"
                ...
                {{ __('media.detail.replace_media_btn') }}
            </button>
        @endif
        <button wire:click="saveMediaDetails" type="button"
            class="inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-primary-600 text-base font-medium text-white hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 dark:focus:ring-offset-gray-800 sm:text-sm">
            {{ __('media.save_changes') }}
        </button>
    </div>
