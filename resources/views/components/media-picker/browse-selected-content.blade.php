@if ($view === 'browse')
    ...
    <div class="mb-4">
        <input type="text" wire:model.live.debounce.300ms="search" placeholder="Search media..."
            class="shadow-sm focus:ring-primary-500 focus:border-primary-500 block w-full sm:text-sm border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-400">
    </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
        @forelse($mediaItems as $media)
            <x-media-picker.media-card :media="$media" mode="picker" :isSelected="$this->isSelected($media->id)"
                wire:click="toggleSelect({{ $media->id }})" :showDetail="false" />
        @empty
