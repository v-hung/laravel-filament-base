(() => {
	...
	function oe(o, t) {
		for (
			...
			e &&
			(e === p.ghost || h(e, 'display') === 'none' || (t && !Xt(e, t)));
		)
			...
			if (n && !f && n.parentNode === r) {
				...
				if (
					...
					(y(l, 'mouseup', i._disableDelayedDrag),
						...
						a.supportPointer &&
							y(l, 'pointermove', i._delayedDragTouchMoveHandler),
		...
		_emulateDragOver: function () {
			if (B) {
				...
				for (
					...
					t &&
					t.shadowRoot &&
					((t = t.shadowRoot.elementFromPoint(B.clientX, B.clientY)),
					t !== e);
				)
					e = t;
			...
			if (
				...
				if (a)
					return (
						...
						X('revert'),
						p.eventCanceled ||
							(at ? S.insertBefore(f, at) : S.appendChild(f)),
						I(!0)
					);
					...
					var gt =
						...
						z =
							...
							(Math.abs(X - r) <= a && V + M < ht) -
								(Math.abs(E - r) <= a && !!V);
					if (!C[c])
						for (var ot = 0; ot <= c; ot++) C[ot] || (C[ot] = {});
